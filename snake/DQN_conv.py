...
#       state.append(d_wall_left)
    ...
    def network(self, weights=None):
        ...
        inp = Input(shape=[self.state_length])
        #feats = Dropout(0.15)(Dense(output_dim = 50, activation='relu')(inp))

        inp_board = Input(shape=[20, 20, 2])
        board = Dropout(0.1)(BatchNormalization()(Conv2D(5,  3, strides=(2,2), activation='relu')(inp_board)))
        board = Dropout(0.1)(BatchNormalization()(Conv2D(7,  3, activation='relu', strides=(2,2))(board)))
        board = Flatten()(Dropout(0.1)(BatchNormalization()(Conv2D(11, 3, activation='relu', strides=(2, 2))(board))))

        feats = Concatenate(axis=1)([inp, board])
        feats = Dropout(0.1)(Dense(100, activation='relu')(feats))
        feats = Dropout(0.1)(Dense(150, activation='relu')(feats))
        feats = Dropout(0.05)(Dense(70, activation='relu')(feats))
        output = Dense(3, activation='softmax')(feats)

        model = Model([inp, inp_board], output)
...
#
    ...
    def replay_new_vectorized(self):
       ...
       feat_minibatch = state_minibatch[:, :self.state_length]
       board_minibatch = state_minibatch[:, self.state_length:]
       board_minibatch = np.reshape(board_minibatch, (-1, 20, 20, 2))
       next_feat_minibatch = next_state_minibatch[:, :self.state_length]
       next_board_minibatch = next_state_minibatch[:, self.state_length:]
       next_board_minibatch = np.reshape(next_board_minibatch, (-1, 20, 20, 2))
       ...
       for i in range(10):
          ...
          target[np.invert(done_minibatch[i*100:(i+1)*100])] = target[np.invert(done_minibatch[i*100:(i+1)*100])] + \
                self.gamma * np.amax(self.target_net.predict([next_feat_minibatch[i*100:(i+1)*100,:], next_board_minibatch[i*100:(i+1)*100,:]]), 1)[np.invert(done_minibatch[i*100:(i+1)*100])]
          target_f = self.policy_net.predict([ feat_minibatch[i*100:(i+1)*100,:], board_minibatch[i*100:(i+1)*100,:] ])
          target_f[:,np.argmax(action_minibatch[i*100:(i+1)*100,:],1)] = target
          self.policy_net.fit([ feat_minibatch[i*100:(i+1)*100,:], board_minibatch[i*100:(i+1)*100,:] ], target_f, epochs=1, verbose=0)
    ...
    def replay_new(self):
        ...
        for state, action, reward, next_state, done in minibatch:
            feat = np.expand_dims(state[:self.state_length], 0)
            board = state[self.state_length:]
            board = np.reshape(board, (1, 20, 20, 2))
            next_feat = np.expand_dims(next_state[:self.state_length], 0)
            next_board = next_state[self.state_length:]
            next_board = np.reshape(next_board, (1, 20, 20, 2))
            target = reward
            if not done:
                target = reward + self.gamma * np.amax(self.target_net.predict([next_feat, next_board])[0])
            target_f = self.policy_net.predict([feat, board])
            target_f[0][np.argmax(action)] = target
            self.policy_net.fit([feat, board], target_f, epochs=1, verbose=0)
